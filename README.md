...
当前智能体框架（如OpenClaw、AutoGPT等）在自主性方面取得了长足进步，但在安全性方面存在根本性缺陷：
...
在CAGE项目的开发过程中，人类作者与人工智能工具（豆包、DeepSeek）协同工作，各自承担了明确的角色：

- **人类作者**：提出项目的核心思想，包括从冷存在模型延伸至智能体安全领域、构建“三层隔离+密码转译”的总体架构、强调“每一次执行都必须使用临时编码”的根本性可控原则，并全程主导方向、评估结果、做出关键决策。
- **豆包AI**：基于人类作者的思路，细化了三层隔离架构的具体描述，设计了密码转译系统的完整工作流程，为后续工程实现提供了清晰的蓝图。
- **DeepSeek**：根据豆包AI的设计，完成了CAGE的代码实现，并参考CEAL的模板撰写了README文档初稿。在人类作者提出“每一步执行都需要临时编码”的改进后，DeepSeek及时调整了代码逻辑，使最终版本符合人类作者的意图。

CAGE项目的所有核心思想、架构决策以及最终代码的审核确认，均由人类作者独立完成。人工智能工具的使用严格限于辅助性工作（如方案细化、代码生成、文档整理），不构成原创性贡献。项目目前处于初步探索阶段，代码与文档将持续在社区协作中迭代完善。